...
class GalaxyShooter {
    ...
    createPlayerBullet() {
        ...
        if (this.player.powerLevel === 1) {
            bullets.push({ x: this.player.x, y: this.player.y - 30, vx: 0, vy: -CONFIG.PLAYER_BULLET_SPEED, type: 'player', width: 5, height: 18, color: '#00ffff', damage: 2 });
        } else if (this.player.powerLevel === 2) {
            bullets.push(
                { x: this.player.x - 15, y: this.player.y - 30, vx: 0, vy: -CONFIG.PLAYER_BULLET_SPEED, type: 'player', width: 5, height: 18, color: '#00ffff', damage: 2 },
                { x: this.player.x + 15, y: this.player.y - 30, vx: 0, vy: -CONFIG.PLAYER_BULLET_SPEED, type: 'player', width: 5, height: 18, color: '#00ffff', damage: 2 }
            );
        } else {
            bullets.push(
                { x: this.player.x, y: this.player.y - 30, vx: 0, vy: -CONFIG.PLAYER_BULLET_SPEED, type: 'player', width: 8, height: 25, color: '#ff00ff', damage: 4 },
                { x: this.player.x - 20, y: this.player.y - 25, vx: -1, vy: -CONFIG.PLAYER_BULLET_SPEED, type: 'player', width: 5, height: 18, color: '#00ffff', damage: 2 },
                { x: this.player.x + 20, y: this.player.y - 25, vx: 1, vy: -CONFIG.PLAYER_BULLET_SPEED, type: 'player', width: 5, height: 18, color: '#00ffff', damage: 2 }
            );
        }
    ...
    createBigBoss() {
        ...
        const baseHp = 70;
        const scaledHp = Math.floor(baseHp * scale);
    ...
    // 大Boss射击模式
    bigBossShoot(boss) {
        // 扇形攻击: 每4秒(240帧)发射5颗子弹
        if (this.frame - boss.lastSpecial > 240) {
            boss.lastSpecial = this.frame;
            // 扇形角度范围：-60度到+60度（共120度）
            for (let i = 0; i < 5; i++) {
                const angle = Math.PI / 2 + (i - 2) * 0.3; // 向下扇形发射
                this.bullets.push({
                    x: boss.x,
                    y: boss.y + 60,
                    vx: Math.cos(angle) * 2,
    ...
    update() {
        ...
        if (this.player) {
            ...
            // 触摸控制 - 战机平滑跟随手指
            if (this.touchActive) {
                const touchSpeed = CONFIG.PLAYER_SPEED * 1.5 * timeScale; // 移动速度不随分数增加
